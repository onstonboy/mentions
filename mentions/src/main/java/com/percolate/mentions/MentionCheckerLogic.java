...
class MentionCheckerLogic {
    ...
    void setMaxCharacters(final int maxCharacters) {
        if (maxCharacters <= 0) {
            throw new IllegalArgumentException(
                    "Maximum number of characters must be greater " + "than 0.");
        }
    ...
    String doMentionCheck() {
        ...
        if (StringUtils.contains(editText.getText(), "@")) {
            ...
            if (searchIsWithinMaxChars(providedSearchText, maxCharacters)
                ...
                if (atSymbolPosition == 0 || spaceBeforeAtSymbol(allTextBeforeCursor,
                        atSymbolPosition)) {
    ...
    /**
     * Check editText has '@' symbol for search
     */
     ...
     */
    private boolean spaceBeforeAtSymbol(final String currentTextBeforeCursor,
            final int atSymbolPosition) {
    ...
    private boolean spaceAfterAtSymbol(final String currentTextAfterCursor) {
        if (currentTextAfterCursor.length() > 1) {
            final char charAfterAtSymbol = currentTextAfterCursor.charAt(1);
            if (Character.isWhitespace(charAfterAtSymbol)) {
                return true;
            }
        }
        return false;
     ...
     * @return true or false
     */
    private boolean searchIsWithinMaxChars(final String providedSearchText,
            final int maxCharacters) {
        return (providedSearchText.length() >= 1 && providedSearchText.length() <= maxCharacters);
    }

    private boolean searchLimitMaxChars(final String providedSearchText,
            final int maxCharacters) {
        return providedSearchText.length() <= maxCharacters;
    }
